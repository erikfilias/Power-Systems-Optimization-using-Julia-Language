clearconsole()
using JuMP, Ipopt, Printf, LinearAlgebra,GLPK
...
#@variable(m, L[Branch.branchnum])
L = zero(1:nbranch)*1e-6
...
for k in 1:nbus
	...
	@constraint(m, Pg[k] - Bus.Pd[k]
	    + sum(P[i] -0.5L[i] for i in in_lines[k])
		- sum(P[i] -0.5L[i] for i in out_lines[k]) == 0)
...
for i in 1:nbranch
	...
	#Pde
	@constraint(m, P[i] == - Branch.a[i]*Branch.b[i]*(th[a]-th[b]+Branch.fi[i]) )

	#@NLconstraint(m, L[i] == Branch.g[i]*(th[a]-th[b]+Branch.fi[i])^2 )
...
# algoritmo de perdidas
for iter in 1:20
	L0 = sum(L[i] for i in 1:nbranch)
	@show L0
	JuMP.optimize!(m)
	...
	@show status
		for i in 1:nbranch
			...
			#@show b
			#@show Branch.g[i]
			#@show cos(value(th[a]) - value(th[b])+ + Branch.fi[i])
			#@show L[5]
		  L[i] = 2*Branch.g[i]*(1-cos(value(th[a]) - value(th[b]) + Branch.fi[i]) )
		  #X = 2*Branch.g[i]*(1-cos(value(th[a]) - value(th[b]) + Branch.fi[i]) )
		  #L[i] = round(X)
		end
	   L1 = sum(L[i] for i in 1:nbranch)
	   @show L1
	   error = abs(L1 - L0)
		 if error < 0.1
			 @show error
			 println(" termino en ", iter," iteraciones")
			 break
	     end
end
...
@printf "-----------------------------------------------------------------------------\n"
@printf "TOTAL"
@printf "%60.4f" float(Sbase*sum((L[i]) for i in 1:nbranch) )
